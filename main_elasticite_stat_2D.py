...
K = np.ma.masked_where(K == 0, K)
plt.matshow(K.todense() if hasattr(K, 'todense') else K, 
           cmap='plasma', 
           fignum=0, 
           aspect='auto')
plt.colorbar(label='Valeur des coefficients')
plt.xlabel('Index des DDL globaux')
plt.ylabel('Index des DDL globaux')

# Optionnel : Ajouter des lignes pour séparer les blocs u_x/u_y
n_total = K.shape[0]
plt.axvline(x=n_nodes-0.5, color='darkblue', linestyle='--', linewidth=1)
plt.axhline(y=n_nodes-0.5, color='darkblue', linestyle='--', linewidth=1)
plt.text(n_nodes/2, -9, 'Bloc u_x', ha='center')
plt.text(n_nodes + n_nodes/2, -9, 'Bloc u_y', ha='center')

plt.savefig('assemblage_matrix.pdf', bbox_inches='tight', dpi=300)
        
K = KK

plt.figure(figsize=(10, 8))
K = np.ma.masked_where(Ke == 0, Ke)
plt.matshow(Ke.todense() if hasattr(Ke, 'todense') else Ke, 
           cmap='plasma', 
           fignum=0, 
           aspect='auto')
plt.colorbar(label='Valeur des coefficients')
plt.xlabel('Index des noeuds locaux')
plt.ylabel('Index des noeuds locaux')

# Optionnel : Ajouter des lignes pour séparer les blocs u_x/u_y
n_total = Ke.shape[0]
plt.axvline(x=n_nodes_elem-.5, color='white', linestyle='--', linewidth=1)
plt.axhline(y=n_nodes_elem-.5, color='white', linestyle='--', linewidth=1)
#plt.text(n_nodes_elem/2, -9, 'Bloc u_x', ha='center')
#plt.text(n_nodes_elem + n_nodes_elem/2, -9, 'Bloc u_y', ha='center')

plt.savefig('Ke_matrix.pdf', bbox_inches='tight', dpi=300)
